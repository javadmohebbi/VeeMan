...
// GetRepositorySummary - Get Repository Summary Overview
func GetRepositorySummary(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("content-type", "application/json")
	...
	if ok := user.GetLoggedIn(context.Get(r, "jwtToken")); ok {
		...
		refs, err := vbemCalls.RepositorySummaryOverview(ls.SessionId)
		if err != nil {
			res := models.ResponseResult{
				Error:  true,
				Result: err.Error(),
