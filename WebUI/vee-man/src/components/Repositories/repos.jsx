...
import {GetReposRelatedToBackupServerListFromServer} from '../../services/repos'
import LightSpinner from '../Loading/Spinner/Light'
import VeeManTable from '../Table/table'
import RepoListRow from './tableRow'
...
const ReposList = (props) => {
  ...
  const containerId = 'reposListRealatedToBackupServer'
  ...
  React.useEffect(() => {
    if (typeof id !== 'undefined') {
      GetReposRelatedToBackupServerListFromServer(id).then(data => {
  ...
  const Thead = () => (
    <>
      ...
      <th scope="col">{t('general.inp.kind')}</th>
      <th scope="col">{t('general.inp.capacity')}</th>
      <th scope="col">{t('general.inp.freeSpace')}</th>
    </>
  )

  const Trows = ({rows=[]}) => {
    return (
      <RepoListRow rows={rows} />
    )
  }
  ...
  return (
      ...
      <div className="row">
        <div className="col-12">
            <h1 className="page-title pb-2">
              ...
              {t('general.nav.repositories')} <> { title !== null ? <span className="text-warning"> ({title}) </span> : null } </>
              {
                busy ?
    ...
    <div className="pg-body">
      ...
      <div className="pg-rest mt-2 mb-2">
        <VeeManTable list={list} listKey={'Repository'} Thead={Thead} Trows={Trows} notAvailableCols={5} />
      </div>
    </div>
...
export default withTranslation()(ReposList)
